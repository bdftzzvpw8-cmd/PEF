...
function load(files) {
  ...
  const withoutVolume = [...accounts.values()].filter(a => a.sources.includes('periodic') && !a.sources.includes('ledger'));
  if (seen.periodic && !seen.ledger) {
    warnings.push('No transaction ledger supplied — the periodic summary carries no '
      + 'wagering volume, so the bonus leaderboard cannot be ranked.');
  } else if (withoutVolume.length) {
    warnings.push(`${withoutVolume.length} account(s) have P&L but no ledger, so no `
      + 'volume: they are excluded from the bonus leaderboard.');
  }
