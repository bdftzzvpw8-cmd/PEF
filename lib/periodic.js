...
function parsePeriodic(rows) {
  ...
  return { accounts, week, weekColumn, groups: [...groups] };
}
...
function verifyPeriodic(rows) {
  ...
  for (const rec of accounts.values()) {
    ...
    if (Math.abs(summed - rec.pnl) > 0.01) {
      issues.push(`${rec.account}: bet-type rows sum to ${summed.toFixed(2)} `
        + `but the account row says ${rec.pnl.toFixed(2)}.`);
    }
  }
  return issues;
}

module.exports = { BET_TYPES, CATEGORIES, categoryOf, parsePeriodic, verifyPeriodic, parseWeekHeader, findWeekColumn };
